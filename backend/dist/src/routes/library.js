...
router.get('/public', async (req, res) => {
    try {
        const q = (req.query.q || 'self help').trim();
        ...
        if (!r.ok)
            throw new Error(`Gutendex error: ${await r.text()}`);
        const json = (await r.json());
        const items = (json.results ?? []).map((b) => {
            const author = b.authors?.[0]?.name || 'Unknown';
            const coverUrl = b.formats?.['image/jpeg'];
            const textUrl = b.formats?.['text/plain; charset=utf-8'] ||
                b.formats?.['text/plain'] ||
                undefined;
            const htmlUrl = b.formats?.['text/html'] || undefined;
            return {
                id: String(b.id),
                title: b.title,
                author,
                coverUrl,
                description: Array.isArray(b.subjects) ? b.subjects.slice(0, 5).join(', ') : undefined,
                linkUrl: b.url,
                textUrl,
                htmlUrl,
            };
...
router.get('/public/:id/content', async (req, res) => {
    try {
        ...
        if (!textUrl && !htmlUrl)
            return res.status(404).json({ error: 'No readable content available' });
        const contentUrl = textUrl ?? htmlUrl;
        const contentRes = await fetch(contentUrl);
        if (!contentRes.ok)
            throw new Error(`Content fetch error: ${await contentRes.text()}`);
...
router.get('/public/:id/meta', async (req, res) => {
    try {
        ...
        const linkUrl = b.url;
        res.setHeader('Cache-Control', 'public, max-age=86400');
        return res.json({ id: String(b.id), title: b.title, author, coverUrl, linkUrl });
    }
    catch (err) {
        // eslint-disable-next-line no-console
        console.error(err);
...
router.get('/public/:id/content-paged', async (req, res) => {
    try {
        ...
        const pageSize = Math.max(1000, Math.min(10000, parseInt(req.query.pageSize || '4000', 10)));
        const now = Date.now();
...
router.get('/videos', async (_req, res) => {
    try {
        ...
        if (apiKey) {
            ...
            if (!ytRes.ok)
                throw new Error(`YouTube API error: ${await ytRes.text()}`);
